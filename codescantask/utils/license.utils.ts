...
export class LicenseUtil {
    ...
    private init(): void {
        if (inputs.COPYLEFT_LICENSE_EXPLICIT) {
            ...
            tl.debug(`Explicit licenses: ${explicitCopyleftLicenses}`);
            this.copyLeftLicenses = new Set<string>(explicitCopyleftLicenses);
            return;
        }

        tl.debug(`Explicit licenses not defined, setting default licenses...`);
        ...
        if (inputs.COPYLEFT_LICENSE_INCLUDE) {
            ...
            tl.debug(`Included copyleft licenses: ${includedCopyleftLicenses}`);
        ...
        if (inputs.COPYLEFT_LICENSE_EXCLUDE) {
            ...
            tl.debug(`Excluded copyleft licenses: ${excludedCopyleftLicenses}`);
